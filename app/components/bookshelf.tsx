...
const Bookshelf = () => {
  ...
  const sortedBooks = [...allBookPosts].sort((a, b) => {
    ...
    return +new Date(b.metadata.dateFinished) - +new Date(a.metadata.dateFinished);
  });
    ...
    <div className="container mx-auto dark:text-white">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {sortedBooks.map((book) => (
          <div
            key={book.slug}
            className={`bg-white shadow-md p-4 dark:bg-gray-800 ${book.metadata.favorite === "true" ? "border-2" : ""}`}
          >
            <h2 className="text-lg font-semibold mb-2 dark:text-neutral-300">{book.metadata.title}</h2>
            <p className="text-sm text-gray-600 mb-2 dark:text-neutral-300">{book.metadata.bookAuthor}</p>
            {book.content && (
              <Link key={book.slug} href={`/bookshelf/${book.slug}`} className="text-blue-500">
                Read Notes
              </Link>
            )}
